...
const HRStaffModuleComplete = () => {
  ...
  const fetchAvailableRoles = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/employees/roles', {
        headers: {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const employeesResponse = await fetch('http://127.0.0.1:8000/api/employees?page=1&limit=100', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/employees?page=1&limit=50', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const fetchMyAttendance = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/employees/my-attendance?user_id=${currentUser.id}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleLeaveAction = async (requestId, action) => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/attendance/leave/admin/action/${requestId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const deactivateEmployee = async (employeeId) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://127.0.0.1:8000/api/employees/${employeeId}/deactivate`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleAttendanceAction = async (action) => {
    ...
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/employees/attendance/checkin', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const uploadEmployeeDocument = async (employeeId, documentName, documentUrl) => {
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://127.0.0.1:8000/api/employees/${employeeId}/documents`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const exportAttendanceData = async () => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/employees/attendance/export?start_date=${startDate.toISOString().split('T')[0]}&end_date=${endDate.toISOString().split('T')[0]}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const syncCollections = async () => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch('http://127.0.0.1:8000/api/employees/sync-collections', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
