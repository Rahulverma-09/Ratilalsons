...
const HRStaffModuleComplete = () => {
  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/employees/dashboard-stats', {
  ...
  const fetchMyAttendance = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/api/employees/${currentUser.user_id}/attendance`, {
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' }
      });
  ...
  const handleAttendanceAction = async (action) => {
    ...
    try {
      const token = localStorage.getItem('access_token');
      const response = await fetch(`http://127.0.0.1:8000/api/employees/${currentUser.user_id}/attendance/${action}`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const submitInstantCheckin = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/employees/${currentUser.user_id}/attendance/checkin`,
        {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  const ProfileManagement = ({ currentUser, fetchCurrentUser, showInfo }) => {
    ...
    const handleEditProfile = async (e) => {
      ...
      try {
        const token = localStorage.getItem("access_token");
        const response = await fetch(
          `http://127.0.0.1:8000/api/employees/${currentUser.user_id || currentUser.id}/profile`,
          {
            method: "PUT",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    const handleUploadDocument = async (event) => {
      ...
      try {
        ...
        const response = await fetch(
          `http://127.0.0.1:8000/api/employees/upload-document?document_type=document`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
    ...
    const handleLeaveSubmit = async (e) => {
      ...
      try {
        const token = localStorage.getItem("access_token");
        const response = await fetch(
          `http://127.0.0.1:8000/api/attendance/leave/admin/all?page=1&limit=20`,
          {
            method: "POST",
            headers: {
              Authorization: `Bearer ${token}`,
