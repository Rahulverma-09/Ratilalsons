...
const MyAttendancePage = () => {
  ...
  const fetchHREmployees = async () => {
    try {
      const token = localStorage.getItem('access_token');
      const res = await fetch('http://127.0.0.1:8000/api/auth/users?limit=200', {
  ...
  const fetchAdminAttendance = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:8000/api/attendance/admin/all?${params}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const handleMarkAttendance = async () => {
    ...
    try {
      ...
      const res = await fetch('http://127.0.0.1:8000/api/attendance/admin/mark-attendance', {
        method: 'POST',
