...
const API_BASE = "http://127.0.0.1:8000/api/generators-utilities";
const USERS_API = "http://127.0.0.1:8000/api/auth/users";
const SITES_API = "http://127.0.0.1:8000/api/sites";
const PAGE_SIZE = 10;
...
const usageFields = [
  ...
  { key: "generator_id", label: "Equipment", type: "select", required: false },
  { key: "vehicle_id", label: "Vehicle", type: "select", required: false },
...
function RecordModal({ type, open, fields, options, record, onSave, onClose, onAddOption }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (type === "usage" && (!data.generator_id || data.generator_id.toString().trim() === "") && (!data.vehicle_id || data.vehicle_id.toString().trim() === "")) {
      setError("Please select at least an Equipment or a Vehicle.");
      return;
    }
    onSave(data);
  };
...
function PaginationControl({ page, setPage, total, colorTheme = 'green' }) {
  ...
  const hasNext = total > safePage * PAGE_SIZE;
  ...
  return (
    <div className="flex items-center justify-between w-full mt-4 px-6 pb-6">
      ...
      <span className="text-lg font-semibold text-gray-700">Page {safePage}</span>
      <motion.button
...
export default function GeneratorsUtilities() {
  ...
  const refreshUsage = (currentUsersMap = usersMap, currentSiteMap = siteMap) => {
    const token = localStorage.getItem("access_token");
    fetch(`${API_BASE}/usage`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) {
          setUsage(data.map(item => mapUsage(item, currentUsersMap, currentSiteMap)));
        }
      })
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const newSiteMap = Object.fromEntries(validSites.map(site => [site.id, site.site_name]));
      const newUsersMap = Object.fromEntries(validUsers.map(u => [u.id, u.full_name]));

      setSites(validSites);
      setSiteMap(newSiteMap);
      setUsers(validUsers);
      setUsersMap(newUsersMap);
      refreshUsage(newUsersMap, newSiteMap);
  ...
  const refreshGenerators = () => {
    ...
    fetch(`${API_BASE}/generators`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setGenerators(data.map(mapGenerator));
      })
      .catch(err => console.error("Generators fetch failed:", err));
  };
  const refreshVehicles = () => {
    ...
    fetch(`${API_BASE}/vehicles`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setVehicles(data.map(mapVehicle));
      })
  ...
  const refreshMaintenance = () => {
    ...
    fetch(`${API_BASE}/maintenance`, { headers: { Authorization: `Bearer ${token}` } })
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data)) setMaintenanceRaw(data);
      })
      .catch(err => console.error("Maintenance fetch failed:", err));
  };
  const refreshReadings = () => {
    ...
    Promise.all([
      ...
      const validSites = Array.isArray(sitesData) ? sitesData : [];
      const validGenerators = Array.isArray(generatorsData) ? generatorsData : [];
      setSites(validSites);
      const generatorList = validGenerators.map(mapGenerator);
      setGenerators(generatorList);
      const siteMapLocal = Object.fromEntries(validSites.map(site => [site.id, site.site_name]));
      ...
      fetch(`${API_BASE}/readings`, { headers: { Authorization: `Bearer ${token}` } })
        .then(res => res.json())
        .then(data => {
          if (Array.isArray(data)) setReadings(data.map(row => mapReading(row, siteMapLocal, generatorMap)));
        });
  ...
  useEffect(() => {
    if (Array.isArray(maintenanceRaw)) {
  ...
  useEffect(() => {
    if (Array.isArray(maintenanceRaw) && maintenanceRaw.length > 0) {
  ...
  const genOptions = useMemo(() => (Array.isArray(generators) ? generators : []).map(g => ({ label: `${g.generator_id} (${g.generator_name})`, value: g.generator_id })), [generators]);
  const vehicleOptions = useMemo(() => (Array.isArray(vehicles) ? vehicles : []).map(v => ({ label: `${v.vehicle_id} (${v.vehicle_name})`, value: v.vehicle_id })), [vehicles]);
  const siteOptions = useMemo(() => (Array.isArray(sites) ? sites : []).map(site => ({ label: site.site_name, value: site.id })), [sites]);
  const operatorOptions = useMemo(() => (Array.isArray(users) ? users : []).map(u => ({ label: u.full_name, value: u.id })), [users]);
  const technicianOptions = operatorOptions;

  // Paging helper with safety checks and automatic page clamping
  const getPage = (arr, tab) => {
    const list = Array.isArray(arr) ? arr : [];
    const totalPages = Math.ceil(list.length / PAGE_SIZE) || 1;
    const rawPage = typeof pages[tab] === "number" && !isNaN(pages[tab]) ? pages[tab] : 1;
        ...
        <AnimatePresence mode="wait">
          {modalType === "generators" && (
            ...
            >
              ...
              <PaginationControl
                ...
                setPage={(val) => setPages((p) => ({ ...p, generators: typeof val === "function" ? val(p.generators || 1) : val }))}
          ...
          {modalType === "vehicles" && (
            ...
            >
              ...
              <PaginationControl
                ...
                setPage={(val) => setPages((p) => ({ ...p, vehicles: typeof val === "function" ? val(p.vehicles || 1) : val }))}
          ...
          {modalType === "usage" && (
            ...
            >
              ...
              <PaginationControl
                page={pages.usage}
                setPage={(val) => setPages((p) => ({ ...p, usage: typeof val === "function" ? val(p.usage || 1) : val }))}
                total={usage.length}
                colorTheme="green"
              />
            </motion.div>
          ...
          {modalType === "readings" && (
            ...
            >
              ...
              <PaginationControl
                page={pages.readings}
                setPage={(val) => setPages((p) => ({ ...p, readings: typeof val === "function" ? val(p.readings || 1) : val }))}
          ...
          {modalType === "maintenance" && (
            ...
            >
              ...
              <PaginationControl
                ...
                setPage={(val) => setPages((p) => ({ ...p, maintenance: typeof val === "function" ? val(p.maintenance || 1) : val }))}
