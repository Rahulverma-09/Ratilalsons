...
const UserDocumentUpload = () => {
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      try {
        ...
        const response = await fetch('http://127.0.0.1:8000/api/employee-docs/employee/my-documents', {
          headers: {
      ...
      const endpoints = [
        `http://127.0.0.1:8000/api/employee-docs/employees/${user.id || user.user_id || user.username}/documents`,
        `http://127.0.0.1:8000/api/api/employees/${user.id || user.user_id || user.username}/documents`,
        `http://127.0.0.1:8000/api/documents?generated_for=${user.id || user.user_id || user.username}`
      ];
  ...
  const handleUploadDocument = async (e) => {
    ...
    try {
      ...
      const uploadUrl = new URL('http://127.0.0.1:8000/api/employee-docs/employee/upload-document');
                    ...
                    <div className="flex gap-2 mt-4 pt-4 border-t border-gray-100">
                      {(() => {
                        ...
                        if (doc.file_path) {
                          ...
                          documentUrl = `http://127.0.0.1:8000/employee_document/${filename}`;

                          console.log('Final document URL:', documentUrl);
                        } else if (doc.stored_filename) {
                          documentUrl = `http://127.0.0.1:8000/employee_document/${doc.stored_filename}`;
                        } else if (doc.pdf_url) {
                          // Ensure pdf_url starts with /
                          documentUrl = `http://127.0.0.1:8000${doc.pdf_url.startsWith('/') ? doc.pdf_url : '/' + doc.pdf_url}`;
                        } else if (doc.document_name || doc.filename) {
                          documentUrl = `http://127.0.0.1:8000/employee_document/${doc.document_name || doc.filename}`;
                        }
                          ...
                          <>
                            <a
                              ...
                              onClick={async (e) => {
                                ...
                                try {
                                  const backendCheck = await fetch('http://127.0.0.1:8000/', { method: 'HEAD' });
                                ...
                                const testUrls = [
                                  'http://127.0.0.1:8000/employee_document/USR-000001_Screenshot%202025-10-28%20112536.png',
                                  'http://127.0.0.1:8000/employee_document/USR-000001_Screenshot 2025-10-28 112536.png',
                                  documentUrl
                                ];
